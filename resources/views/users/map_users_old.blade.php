...
@section('title') Map Users @endsection
@section('content')

      <section class="content-header">
      <h1>
            Map Users to HR Business Partners (
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                            ...
                                            <div class="form-group{{ $errors->has('user_id') ? ' has-error' : '' }}"">
                                                    <label>HR Business Partner (HBP) </label>
                                                    <select onchange="FilterByStaffCategory(this.value)" name = 'hr_user_id' id = 'hr_user_id' class = 'form-control' required>
                                                    <option value="">Select HR Business Partner</option>
                                                    
                                                    @foreach($get_hr_Staff as $hr) 
                                                    
                                                        @if (Request::old('user_id') == $hr->sap)
                                                    <option {{$hr_id==$hr->user_id?" selected ": ""}} value="{{ $hr->sap }}" >{{$hr->name}}</option>
                                                        @else
                                                            <option {{$hr_id==$hr->sap?" selected ": ""}} value="{{$hr->sap}}">{{$hr->name}}</option>
                                                        @endif
                                                    @endforeach 
                                                    </select>
                                            </div>
                                    ...
                                    <div class="col-md-3">
                                      ...
                                      <div class="form-group{{ $errors->has('user_id') ? ' has-error' : '' }}"">
                                        ...
                                        <select {{isset($request->hbp_id)?"":"disabled"}} onchange="filter_by_dept(this.value)" name = 'department_id' id = 'department_id' class = 'form-control' >
                                        <option value="30221222211">Select a Department</option>
                                        
                                        @foreach($department as $key => $value) 
                                            @if (Request::old('user_id') == $key)
                                        <option {{$request->dept == $key?" selected ": ""}} value="{{ $key }}" >{{$value}}</option>
                                            @else
                                                <option {{$request->dept == $key?" selected ": ""}} value="{{$key}}">{{$value}}</option>
                                            @endif
                                        @endforeach 
                                    ...
                                    <div class="col-md-3">
                                                    <button style="margin-top:25px" type="submit" class="btn btn-primary block">Map to HR Business Partner</button>
                                    </div>
                                </div>
                   ...
                   @foreach ($users as $a => $user)  
                        <tr>
                                ...
                                <td>
                                        ...
                                        <label class="custom-control-label"> <input {{$hr_id ==$user->mapped_to_userid?"checked":""}} type="checkbox" name="staff_id[]" value="{{$user->id}}" class="custom-control-input">
                                            &nbsp;{{$user->name}}</label>
                                        </div>
                                </td>
                                <td>{{$user->sap}}</td>
...
function FilterByStaffCategory(value)
{
  window.location.href = SITEURL+"/map_users?hbp_id="+value+"&category={{$request->category}}&dept={{$request->dept}}";
  
}

function filter_by_dept(value)
{
  

  var hr_id = $('#hr_user_id').val();


  window.location.href = SITEURL+"/map_users?hbp_id="+hr_id+"&category={{$request->category}}&dept="+value+"";
