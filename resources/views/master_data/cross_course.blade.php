	...
	<div class="content-box">
		<div class="element-wrapper">
			<h6 class="element-header">
				All Cross Course List (@if( $cross_course_collection->isEmpty() ) 0 @else{{$cross_course_collection->count()}}@endif)
			</h6>
			...
			@if(Session::get('cross_course_error'))
			...
			<div class="element-box">
				<div class="table-responsive">
					...
					<table id="dataTable1" class="table table-striped" style="width: 100% !important;">
						<thead>
							<tr>
                                @if(Route::current()->getName() == 'edit_cross_course')
								<th></th>
                                @endif
								
								<th>Programme</th>
								<th>Course</th>
								<th>Cross Course Category</th>
								<th>Cross Course Unit</th>
								<th>Cross Course Year</th>
								<th>Cross Semester</th>
						...
						@foreach($cross_course_collection as $val)
							<tr>
								...
								<td>
										...
										<i title="Edit {{ $val->course_title }}" style="cursor: pointer" data-id="{{ $val->cross_course_id }}" class="fa fa-edit"></i>
								</td>
								...
								<td>{{$val->cross_course_year}}</td>
								<td>{{$val->cross_course_semester}}</td>
						     </tr>
						@endforeach
						</tbody>
					</table>
				...
				$.ajax(
						{
							...
							@if(Route::current()->getName() == 'edit_cross_course')
							url:"{{ route('dynamic_cross_course_edit') }}",
							@else
							url:"{{ route('dynamic_cross_course_edit') }}",
							@endif
							beforeSend:function()
