...
@if ($eligible_for_ca > 0)
	...
	<div class="content-box">
		<div class="element-wrapper">
			<h6 class="element-header">
				Available Assessement List (@if( $assessement_collection->isEmpty() ) 0 @else{{$assessement_collection->count()}}@endif)
			</h6>
			@if(Session::get('news_success'))
			<div class="alert alert-success" style="margin-top:3px; margin-bottom:0">
				Assessement Updated successfully!
			</div>
			<br/>
			@endif
					...
					<table id="dataTable1" class="table table-striped" style="width: 100% !important;">
						...
						@foreach($assessement_collection as $val)
							<tr>
								...
								@if(substr($val->start_date,0,10)." ".$val->start_time < date('Y-m-d H:i:s')) {{-- Check if the test is opened --}}
										@if(!isset($completed_assessement_collection[$val->assessment_id])) 
											
										   @if(date('Y-m-d H:i:s') < substr($val->expiration_date,0,10)." ".$val->expiration_time)
							                     <td><a href="{{route('take_ca_questions').'?id='.base64_encode($val->assessment_id)}}" class="btn btn-info">Take C.A</a></td>
											@else
							                    <td></td>
										    @endif
						...
						@endforeach
						{{-- @foreach($pending_assessement_collection as $val)
							<tr>
								<td>{{$val->course_title}}</td>
								<td>{{$val->short_code}}</td>
								<td>{{$val->session_name}}</td>
								...
								<td>{{substr($assessement_exp_date[$val->short_code],0,10)}}</td>
								<td>{{$assessement_exp_time[$val->short_code]}}</td>
	...
	@else
		<div align="center">
			...
			<p> Assessement is only available to students who has done course registration<p>
		</div>
    @endif
