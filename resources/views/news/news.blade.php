	...
	<div class="content-box">
		<div class="element-wrapper">
			<h6 class="element-header">
				All News List (@if( $news_collection->isEmpty() ) 0 @else{{$news_collection->count()}}@endif)
			</h6>
			...
			<div class="element-box">
				<div class="table-responsive">
					...
					<table id="dataTable1" class="table table-striped" style="width: 100% !important;">
						<thead>
							<tr>
                                @if(Route::current()->getName() == 'edit_news')
								<th></th>
                                @endif
								<th></th>
								<th>Title</th>
						...
						@foreach($news_collection as $val)
							<tr>
								...
								<td>
										...
										<i title="Edit {{ $val->news_title." ".$val->news_title }}" style="cursor: pointer" data-id="{{ $val->news_id }}" class="fa fa-edit"></i>
								</td>
                                @else
                                    {{--<td>
                                ...
                                @endif
								<td class="center"><img class="img-rounded" height="30" width="30" src='{{ asset("images/news/".$val->img_path) }}'/></td>
								<td>{{$val->news_title}}</td>
								
								<td>{{$val->created_at}}</td>
								<td>{{$val->user_id}}</td>
							<!--	<td> <a href="">{!!$val->status ==1?"<font color='green'><i class='fa fa-cog'></i> Enabled</font>":"<font color='red'><i class='fa fa-cog'></i> Disabled</font></font>"!!}</a> </td>  -->
							</tr>
						@endforeach
						</tbody>
					</table>
				...
				$.ajax(
						{
							...
							@if(Route::current()->getName() == 'edit_news')
							url:"{{ route('dynamic_news_edit') }}",
							@else
							url:"{{ route('dynamic_user_view') }}",
							@endif
							beforeSend:function()
							{
