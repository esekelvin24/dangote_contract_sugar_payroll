                ...
                @if(count($menu_header) > 0)
                    @foreach ($menu_header as $header)
                        ...
                        <li class="navigation-header">
                            ...
                            @if(count($parent) > 0)
                                @foreach ($parent as $menu_one)
                                   @if($menu_one->category_header == $header->id && $menu_one->menu_level == 0)

                                        <li>
                                               @php
                                                  $route_id = explode("~",Request::route()->getName()) ;
                                                  $url_parent = url($menu_one->menu_url);
                                               @endphp 
                                            <a id="{{$menu_one->menu_id}}" {!! $route_id[0]==$menu_one->menu_id?'class="active"':"" !!} {!! $menu_one->sub_menu=='true'?'href="javascript:void(0);"':'href="'.$url_parent.'"'!!} data-toggle="collapse" data-target="#menu_id{{$menu_one->menu_id}}"><div class="pull-left"><i class="{{$menu_one->icon_class}}"></i><span class="right-nav-text"> {{$menu_one->menu_name}}</span></div><div class="pull-right">{!! $menu_one->sub_menu=='true'?'<i class="zmdi zmdi-caret-down"> </i>':''!!}</div><div class="clearfix"></div></a> 
                                                  
                                                     

                                                <ul id="menu_id{{$menu_one->menu_id}}" class="collapse collapse-level-1">

                                                    @foreach ($parent as $sub_menu_one)
                                                    
                                                       @if($sub_menu_one->parent_id == $menu_one->menu_id && $sub_menu_one->menu_level == 1)
                                                            
                                                       <li>
                                                        @php
                                                            $first_url = url($sub_menu_one->menu_url);
                                                        @endphp
                                                       <a class="active text-white" {!! $sub_menu_one->sub_menu == 'true'?'href="javascript:void(0);"': 'href="'.$first_url.'"' !!}  data-toggle="collapse" data-target="#dashboard_dr{{$sub_menu_one->menu_id}}"><div class="pull-left"><span class="right-nav-text">{{$sub_menu_one->menu_name}}</span></div><div class="pull-right">{!! $sub_menu_one->sub_menu == 'true'?'<i class="zmdi zmdi-caret-down"></i>':'' !!}</div><div class="clearfix"></div></a>    
                                                           
                                                         @if($sub_menu_one->sub_menu == "true")
                                                            <ul id="dashboard_dr{{$sub_menu_one->menu_id}}" class="collapse collapse-level-1">
                                                                
                                                              @foreach ($parent as $sub_menu_two)
                                                               @if($sub_menu_two->parent_id == $sub_menu_one->menu_id)
                                                                    <li>
                                                                        @php
                                                                            $second_url = url($sub_menu_two->menu_url);
                                                                        @endphp
                                                                        <a class="active-page text-white" href="{{$second_url}}">{{$sub_menu_two->menu_name}}</a>
                                                                    </li>
                                                                @endif
                                                              @endforeach
                                                            </ul>
